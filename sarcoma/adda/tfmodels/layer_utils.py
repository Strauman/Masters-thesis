...
def print_summary(model, line_length=None, positions=None, print_fn=None):
    ...
    if sequential_like:
        ...
        # positions = positions or [.25, .5, .55, .7, 1.]
        positins=[.25, .5, .55, .7, 1.]
        if positions[-1] <= 1:
            positions = [int(line_length * p) for p in positions]
        # header names for the different log elements
        to_display = ['Layer (type)', 'Output Shape', 'Param #', 'Connected to', 'Activation']
        relevant_nodes = []
    ...
    def print_layer_summary(layer):
        ...
        fields = [name + ' (' + cls_name + ')', output_shape, layer.count_params(), activation]
