...
DO_SUMMARY = ("--summary" in sys.argv or "--keras-summary" in sys.argv)
# Model setup
...
summary_out_str = ""
def build_graph(cfg=None, do_shuffle=500):
    ...
    if "--delete-tb" in sys.argv and "ON_SERVER" in os.environ:
        ...
        if "--run-name" in sys.argv:
            ...
            color_print("NOT DELETING TB (no --run-name given)", style="danger")
    ...
    if do_shuffle:
        DS_s.ds_send_tr(shuffle=do_shuffle)
    DS_s.ds_send_val(batch=val_s_batch_size)
    if cfg.trcfg.prefetch is not False:
        DS_s.ds_send_tr(("prefetch", dict(buffer_size=cfg.trcfg.prefetch)))
    ...
    if do_shuffle:
        DS_t.ds_send_tr(shuffle=do_shuffle)
    DS_t.ds_send_val(batch=val_t_batch_size)
    if cfg.trcfg.prefetch is not False:
        DS_t.ds_send_tr(("prefetch", dict(buffer_size=cfg.trcfg.prefetch)))

    if cfg.trcfg.ds_s_val_send:
        for tosend in cfg.trcfg.ds_s_val_send:
            DS_s.ds_send_val(tosend)
    if cfg.trcfg.ds_t_val_send:
        for tosend in cfg.trcfg.ds_t_val_send:
            DS_t.ds_send_val(tosend)
    ...
    # Make savers
    cfg.models_dir = cfg.models_dir
    cfg.model_save_root = model_save_root = os.path.join(cfg.models_dir, cfg.model_save_subdir)
    _savers = hlp.savers_from_cfg(cfg)
    # _savers = util.Dottify(
    #     classifier=new_saver("classifier", cfg.models_dir, save_dir_name=cfg.source_dataset, restore_dir_name=cfg.source_dataset),
    #     source_map=new_saver("source_map", cfg.models_dir, save_dir_name=cfg.source_dataset, restore_dir_name=cfg.source_dataset, state=cfg.trainers.pretrain.state),
    ...
    #     cross_src_target=new_cross_saver(name="cross_src_target", save_root=cfg.models_dir, source_scope="source_map", dest_scope="target_map", save_dir_name=cfg.target_dataset, restore_dir_name=cfg.source_dataset, state=cfg.trainers.pretrain.state)
    # )
    # savers=_savers
    # cfg.savers=savers
    cfg.savers = _savers
    ...
    def keras_accummulate_summary_out(mod):
        if hasattr(mod,"keras_summary"):
            mod.keras_summary(line_length=int(sh_columns * 0.8), positions=default_positions)
        else:
            mod.summary(line_length=int(sh_columns * 0.8), positions=default_positions)
    ...
    def summarize(do_print=False):
        ...
        summary_args=dict(summary_func=pr_accumulate_summary_out, include_sanity=False, header_print_func=accumulate_summary_header)
        if "--keras-summary" in sys.argv:
            summary_args=dict(summary_func=keras_accummulate_summary_out, include_sanity=False, header_print_func=accumulate_summary_header)



        get_summaries(**summary_args)
    ...
    return DS_s, DS_t, cfg, tensorboard_path, CONFIG_NAME
